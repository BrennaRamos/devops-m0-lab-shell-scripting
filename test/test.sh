...
do
  case $line_number in 
    ...
    2)
      if [[ "$line" == *"high"* || "$line" == *"lower"* ]]; then
        lg -r p -m "Third attempt, too high"
      else
        lg -r f -m "Third attempt, not too high" -e "Random number was 25. Tested with 20. Make sure your game outputs a message that the attempt was *too high*, or that the user should try a *lower* number."
      fi
      if [[ "$line" == *"5"* && "$line" == *"4"* ]]; then
    ...
    3)
      if [[ "$line" == *"high"* || "$line" == *"lower"* ]]; then
        lg -r p -m "Fourth attempt, too high"
      else
        lg -r f -m "Fourth attempt, not too high" -e "Random number was 25. Tested with 10. Make sure your game outputs a message that the attempt was *too high*, or that the user should try a *lower* number."
      fi
      if [[ "$line" == *"5"* && "$line" == *"4"* ]]; then
